...
| BUG-031 | P2 | Profit Calculator | FBA storage hardcoded to one month at ₹45/cu ft, and a low-margin banner asserting "15–25% is usually a sustainable range", both presented as facts | expose as assumptions | hidden assumptions | **FIXED** — margin band removed; FBA storage is now a "Months held in FBA storage" input, defaulting to 1 |
...
| BUG-043 | **P1** | Price Band Optimizer | Third tool on the residual-contribution base — and the one where it does most damage, because this tool **compares** prices. Electronics, cost ₹700, ACOS 40%, planned ₹1,100: old model recommended **₹1,100 with "net ₹70.13"**; corrected, the best price is **₹1,000 and still loses ₹281.38** | one model across all three pricing tools | a different price recommended, and a ₹70 profit shown where there is a ₹281 loss | **FIXED** |
| BUG-044 | P2 | Launch Budget | "A typical launch leans heaviest on PPC early, then shifts toward coupons once reviews start landing" — an unsourced claim about what is typical, in a tool that only sums the seller's own inputs | report the arithmetic entered | unsourced norm | **FIXED** |
| BUG-045 | P2 | Restock Planner | The reorder point was computed and never displayed — the most actionable figure the tool holds | surface it | dead output | **FIXED** — now a KPI, "Reorder at 45 units left" |
| BUG-046 | **P1** | Price Band Optimizer | With every tested price loss-making it said *"Dropping to ₹1,000 would **earn** ₹35.58 MORE per unit"*. It does not earn more — it loses ₹35.58 less | switch the whole card to least-loss language | a loss reported as an earning | **FIXED** |
| BUG-047 | **P1** | Product Launch Budget | A ₹5,000 budget line labelled **"Samples & Reviews"** — ambiguous next to Amazon's prohibition on offering benefits in exchange for reviews. A compliance liability, not a wording problem | name it "Product samples" and state the rule | the tool appeared to budget for paid reviews | **FIXED** |
| BUG-048 | P2 | Price Band Optimizer | Lede dated the broad under-₹1,000 zero-referral change to **10 June 2026**; that date updated *specific* categories, and "most categories" was too broad | wording that stays correct as category rules change | a dated claim that rots | **FIXED** |
| BUG-049 | P2 | Product Launch Budget | Independently-rounded allocations summed to **101%** | one decimal | 52+26+14+9 = 101 | **FIXED** |
| BUG-050 | P2 | Restock Planner | Subtitle promised *"so you never stock out"* — a guarantee a simple average cannot make; zero daily sales rendered **"∞" cover and a 0 suggested quantity** | estimate, and "Not measurable" | a guarantee plus zero-vs-unknown | **FIXED** |
| PRIV-1 | **P1** | Account / all tools | Logout cleared the auth token but **not** tool data — `sku_costs`, `gstinv_seller` (business name, address, **GSTIN**), `fnsku_rows`, `rec_done`, `fb_weights` survived in `localStorage` on a shared machine. Raised to P1: the GST Invoice Generator stores a GSTIN and business address **with no login at all**, so logout never fired for those users | clear on logout **and** give an explicit control that does not require an account | a GSTIN readable by the next person on the machine | **FIXED** |
| BUG-051 | **P0** | App-wide *(introduced and caught during the PRIV-1 fix)* | The first version put `TOOL_DATA_KEYS` at the top of the script referencing key constants declared ~3,000 lines later. `const` hoists into the temporal dead zone, so the array's initializer threw at load and **killed the entire script** — every tool rendered blank | build the list at call time | total app failure; `node --check` passed, only the browser test caught it | **FIXED** — regression test added |
